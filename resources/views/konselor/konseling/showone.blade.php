                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-body box-profile">
                            ...
                            <ul class="list-group list-group-unbordered mb-3">
                                ...
                                <li class="list-group-item">
                                    <b>TTL</b> <a class="float-right">{{$konseling->mahasiswa->tempat_lahir}},
                                        {{$konseling->mahasiswa->tanggal_lahir}}</a>
                                </li>
                                <li class="list-group-item">
                                    <b>Telepon</b> <a class="float-right"
                                        href="tel:{{$konseling->mahasiswa->nomor_hp}}">{{$konseling->mahasiswa->nomor_hp}}</a>
                                </li>
                                <li class="list-group-item">
                                    <b>Email</b> <a class="float-right"
                                        href="mailto:{{$konseling->mahasiswa->email}}">{{$konseling->mahasiswa->email}}</a>
                                </li>
                                <li class="list-group-item">
                                    <b>Alamat</b> <a class="float-right">{{$konseling->mahasiswa->alamat}},
                                        {{$konseling->mahasiswa->kota}}</a>
                                </li>
                            </ul>
                        </div>
                        <!-- /.card-body -->
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea name="keterangan" class="form-control" rows="3"
                                                placeholder="Tulis keterangan atau biarkan kosong."
                                                <?php if($konseling->status == 'succeed' || $konseling->status == 'canceled') echo "disabled"; ?>>{{$konseling->keterangan}}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12">
                                        <!-- textarea -->
                                        <div class="form-group">
                                            <label>Laporan</label>
                                            <textarea name="laporan_teks" class="form-control" rows="5"
                                                placeholder="Anda belum menulis laporan. Tulis laporan setelah melakukan konfirmasi konseling."
                                                <?php if($konseling->status == 'created' || $konseling->status == 'rescheduled' || $konseling->status == 'succeed' || $konseling->status == 'canceled' || $konseling->status == 'rescheduled-by-counselor' ) echo "disabled"; ?>>{{$konseling->laporan_teks}}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12">
                                        <!-- textarea -->
                                        ...
                                        <select value="{{$konseling->kategori_id}}" class="form-control custom-select" name="kategori_id">
                                                <option value="">-- Tentukan Kategori Masalah --</option>
                                                @foreach ($listKategori as $kategori)

                                                    @if ($konseling->kategori_id == $kategori->id)
                                                        <option value="{{$kategori->id}}" selected>{{$kategori->name}} (Terpilih)</option>
                                                    @else
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control custom-select" name="status"
                                                <?php if($konseling->status == 'succeed' || $konseling->status == 'canceled') echo "disabled"?>>
                                                @if($konseling->status == 'created' || $konseling->status ==
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <button type="submit" class="btn btn-primary" name="button"
                                                <?php if($konseling->status == 'succeed' || $konseling->status == 'canceled') echo "disabled"?>>Simpan</button>
                                        </div>
                                    </div>
                                    @endif
                                </div>
